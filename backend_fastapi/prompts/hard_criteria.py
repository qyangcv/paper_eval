...
从以上内容中，筛选15至20条最重要的错误，且保证每个摘要、Abstract和每个章节都至少有1个问题。
问题按重要程度排序为：
高：主观用词("我们"、"我") 、错别字(中文错别字，英文常用单词拼写错误)
中：逻辑混乱(主要是长句式逻辑不清)、 搭配不当(中文词语搭配不当)
低：指代不清(代词模糊指代不清)、 标点混用(只关注逗号顿号混用的情况)
筛选时按高>中>低的顺序进行选择。
...
[
    {{  
        "chapter": <章节名>,
        "sub_chapter": <小节名>,
        "severity": <问题指标，从且仅从以下选项中选择：高、中、低，“主观用词”和“错别字”对应高，“逻辑混乱”和“搭配不当”对应中，“指代不清”和“标点混用”对应低>,
    ...
    {{
        "chapter": <章节名>,
        "sub_chapter": <小节名>,
        "severity": <问题重要程度，从且仅从以下选项中选择：高、中、低，“主观用词”和“错别字”为高，“逻辑混乱”和“搭配不当”为中，“指代不清”和“标点混用”为低>,
        "original_text": <问题所在原文完整句子>,
        "detail": <存在的问题，具体描述、指出出现问题的字词>,
        "type": <问题所属类型，从且仅从以下选项中选择：主观用词、错别字、逻辑混乱、搭配不当、指代不清、标点混用>,
        "suggestion": 建议修改为<修改后的完整句子>
